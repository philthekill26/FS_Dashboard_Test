...
dataset2 = pd.read_excel('Managed Sellers DB.xlsx',
                         usecols='Q:R')
dataset3 = pd.read_excel('Top 30 Seller GTV.xlsx',
                         usecols='A:B')

# emojis: https://www.webfx.com/tools/emoji-cheat-sheet/
st.set_page_config(page_title="Main Dashboard", page_icon=":bar_chart:", layout="wide")

# Add logo or icon to the sidebar using a URL
st.sidebar.image("data/logo.png", caption="")
st.sidebar.header("Main Dashboard")
...
pie_chart1 = px.pie(dataset2,
                    title='Sellers Per Vertical',
                    values='Sellers',
                    names='Verticals')

col1.plotly_chart(pie_chart1)

pie_chart2 = px.pie(dataset3,
                    title='Top 30 Sellers GTV ($)',
                    values='TTM GTV ($)',
                    names='Company ID')
